// Namespace
var android = android || {};
android.selection = android.selection || {};
...
android.selection.currentPage = 0;
...
android.selection.getHandleBounds2 = function(range){
	...
	var boundingRect = {left:Math.round(range.getBoundingClientRect().left) , top:Math.round(range.getBoundingClientRect().top) , right: Math.round(range.getBoundingClientRect().right) , bottom:Math.round(range.getBoundingClientRect().bottom)};	
	//alert(boundingRect);
	return boundingRect;
};
/**
...
//Final Checkup to Clear Unreasonable User Selection
		//alert('_dotNet : ' + _dotNetWindowPaddingLeft + ',' + _dotNetWindowPaddingTop + ',' + _dotNetWindowPaddingRight + ',' + _dotNetWindowPaddingBottom);
		//alert('android.selection.page : ' + android.selection.pageWidth + ',' + android.selection.pageHeight);		
		
		if(handleBounds.left < 0 || handleBounds.right > android.selection.pageWidth + _dotNetWindowPaddingLeft + _dotNetWindowPaddingRight || handleBounds.top < 0 || handleBounds.bottom > android.selection.pageHeight + _dotNetWindowPaddingTop + _dotNetWindowPaddingBottom)
		{
			//alert('HANDLEBOUNDS : ' + handleBounds.left + ',' + handleBounds.top + ',' + handleBounds.right + ',' + handleBounds.bottom );
			var selection = window.getSelection();
			selection.removeAllRanges();
			window.FORM.returnSelectionStartAndEnd(android.selection.webviewName, result.start, result.start, event.clientX, event.clientY, "", "");
...
android.selection.addNoteMark = function(start, end, left, top){
	...
	else//(android.selection.leftToRight == false)
	{
		if(android.selection.webviewName == 'LEFT_WEBVIEW')
		{	
			//alert('L_W');			
...
android.selection.addHtml5Audio = function(mediaFilename, cssClassName){
